...
begin
    ...
    while row_index < row.size do
    	...
    	while col_index < col.size do
            ...
            con.exec "UPDATE #{filename} SET #{att_name[col_index]}='#{col[col_index]}' WHERE id=#{row_index}"
            puts "UPDATE #{filename} SET #{att_name[col_index]}='#{col[col_index]}' WHERE id=#{row_index}"
    		col_index += 1
    	end
    	row_index += 1
        id += 1
